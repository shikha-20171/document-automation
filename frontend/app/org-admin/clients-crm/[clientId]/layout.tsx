...
import {
  ...
  Globe, Mail, Phone, MapPin, ChevronDown, CheckCircle2, X, RotateCcw,
} from "lucide-react";
...
const TABS = [
  ...
  { label: "Requests & Tasks", path: "requests" },
...
export default function ClientDetailLayout({ children }: { children: React.ReactNode }) {
  ...
  const [completedReqCount, setCompletedReqCount] = useState<number>(0);
  ...
  const loadData = useCallback(async () => {
    ...
    const fetched = await clientStore.fetchClientById(clientId);
    setClient(fetched);
    ...
    // 3. Fetch requests count
    const reqs = await clientStore.fetchRequests(clientId);
    setOpenReqCount(reqs.filter(r => ["New", "In Progress", "Waiting for Client", "Pending Approval"].includes(r.status)).length);
    setCompletedReqCount(reqs.filter(r => r.status === "Completed").length);
    setLoading(false);
  }, [clientId]);
  ...
  const handleRestore = async () => {
    ...
    await loadData();
    showToast(`Client "${client.name}" restored to Active`);
    setMoreOpen(false);
  };
  ...
  if (!client && !loading) {
    return (
      <div className="flex flex-col items-center justify-center py-24 text-slate-400">
        ...
        <button onClick={() => router.push("/org-admin/clients-crm")} className="mt-4 text-xs font-bold text-[#274690] hover:underline">
          ← Back to Clients
        </button>
      </div>
    );
  ...
  const summaryStats = [
    { label: "Total Documents", value: docCount },
    { label: "Open Requests / Tasks", value: openReqCount },
    { label: "Completed Requests", value: completedReqCount },
    { label: "Industry", value: client?.industry || "General" },
    { label: "Department", value: client?.department || "Operations" },
  ];
            ...
            <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
              <div className="flex items-start gap-4">
                ...
                <div>
                  <div className="flex items-center gap-2.5 flex-wrap">
                    ...
                    <span className={`inline-flex items-center rounded-full border px-2.5 py-0.5 text-[10px] font-bold ${STATUS_STYLES[client.status] || "bg-slate-100 text-slate-700"}`}>
                      {client.status}
                    </span>
                  </div>
              ...
              <div className="flex gap-2 flex-wrap">
                ...
                <div className="relative">
                  ...
                  {moreOpen && (
                    <div className="absolute right-0 top-full mt-1 w-48 rounded-xl border border-slate-200 bg-white shadow-xl z-20 py-1">
                      <button onClick={() => { router.push(`/org-admin/clients-crm/${clientId}/notes`); setMoreOpen(false); }} className="flex w-full items-center px-3 py-2 text-xs font-semibold text-slate-700 hover:bg-slate-50">
                        Add Note
                      </button>
                      <button onClick={() => { router.push(`/org-admin/clients-crm/${clientId}/activities`); setMoreOpen(false); }} className="flex w-full items-center px-3 py-2 text-xs font-semibold text-slate-700 hover:bg-slate-50">
                        View Activities
                      </button>
                      <div className="border-t border-slate-100 mt-1 pt-1">
                        {client.status !== "Archived" ? (
                          <button onClick={handleArchive} className="flex w-full items-center px-3 py-2 text-xs font-semibold text-amber-700 hover:bg-amber-50">
                            Archive Client
                          </button>
                        ) : (
                          <button onClick={handleRestore} className="flex w-full items-center px-3 py-2 text-xs font-semibold text-emerald-700 hover:bg-emerald-50">
                            Restore to Active
                          </button>
                        )}
                      </div>
